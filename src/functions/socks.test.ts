...
describe("CRUD interface LocalStorag", () => {
  ...
  test("make sock model4", () => {
    let s = new Sock(35, 10, 10);
    console.log(s.description.length);
    let r = s.getCurrentRow();
    for (let i = 0; i < 1000; i++) r = s.getCurrentRow();
    expect(s.currentRow).toBe(s.description.length - 1);
    expect(r.desc).toStrictEqual(
      `Убавляем, пока на спицах не останется 8 петель. Их стягиваем на нитку `,
    );
  });
  test("make sock model4", () => {
    let s = new Sock(35, 10, 10);
