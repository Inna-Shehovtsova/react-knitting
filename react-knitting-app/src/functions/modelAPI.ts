...
export async function getModelList(name:string){
    ...
    if(m){
        ...
        let found:localSSave|undefined = m1.find(value=>value.username === name);
        if(found){
...
export async function setModelList(model:TSockSaveNamed){
    ...
    let l = new Array<TSockSave>();
    let temp:TSockSave = {row:model.row,
         stich:model.stich,
          progress:model.progress,
        size: model.size , name:model.name};
    l.push(temp);
...
export async function updateModel(model:TSockSaveNamed){
    ...
    if(m){
        let m1:Array<localSSave> = JSON.parse(m);
        let found:number = m1.findIndex(value=>value.username === model.username);
        if(found > -1){
            let i = m1[found].projects.findIndex(value => value.name ===model.name);
            if(i > -1){
