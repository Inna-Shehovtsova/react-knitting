...
import { setSize, setRow,setStich,setModel,getNextRow,getPreviosRow,setName,setAll } from "../redux/sockSlice";
...
export const SockInput:FC =  ()=>{
    ...
    const isAutorized = useSelector((state:RootState )=>state.login.isAuthorized);
    ...
    if(isAutorized){
        ...
        return  (<main>
            <div><div><p>Сохранненные проекты</p>
            <ul >
        {loading ? <li>Проекты в загрузке</li>: models.map((m, i) => (
          <li 
          onClick={ (event:React.MouseEvent<HTMLLIElement>)=>{dispatch( chekModel({i}) ) ;
          dispatch(setAll(m));
        }}
            >{m.name} р.{m.size} {m.stich}/{m.row} прогресс {m.progress}</li>
        ))}
      </ul></div>
